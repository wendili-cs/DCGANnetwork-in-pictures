...
toShow = True #训练途中是否展示当前成果
howManyShow = 20 #每迭代多少次展示一次
howManySave = 2
...
output_path = "trained_model/model.ckpt"
total_epoch = 500
batch_size = 100 #决定生成品的特异性，越小越具有特异性
    ...
    if toTrain:
        for epoch in range(total_epoch):
            ...
            if epoch % (10*howManyShow) == 0 and toShow:
                ...
                #生成图像
                samples = sess.run(generator_output,feed_dict={z:noise, keep_prob: 1.0})
                fig,a = plt.subplots(show_batch_row,show_batch_col)
                for i in range(show_batch_row):
                    for j in range(show_batch_col):
                        a[i][j].clear()
                        a[i][j].set_axis_off()
                        a[i][j].imshow(np.reshape(samples[show_batch_row*i + j],(n_height,n_width,3)))
                plt.draw()
                plt.show()
    else:
        for t in range(do_times):
            ...
            noise = np.random.normal(size=(new_batch,n_noise))
            #生成图像
            samples = sess.run(generator_output,feed_dict={z:noise, keep_prob: 0.8})
            ...
            plt.savefig('samples/output_{0}.png'.format(t))
            plt.close(fig)
